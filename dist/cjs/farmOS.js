...
var chain = require('ramda/src/chain.js');
var compose = require('ramda/src/compose.js');
var concat = require('ramda/src/concat.js');
...
var path = require('ramda/src/path.js');
...
var filter = require('ramda/src/filter.js');
...
const getDefinition = (schema, pointer) => {
  ...
  const subschema = path__default["default"](pathSegments, schema);
...
const dropMilliseconds = replace__default["default"](/\.\d\d\d/, '');
...
const removeDrupalMetaSchemas = drupalFields => evolve__default["default"]({
  properties: omit__default["default"](drupalFields),
  required: filter__default["default"](field => !drupalFields.includes(field)),
});
...
const transformD9Schema = entName => (d9Schema) => {
  ...
  const transformAttributeFields = evolve__default["default"]({
    properties: mapObjIndexed__default$1["default"]((subschema, propName) => {
      const hasOwnDefault = isObject(subschema) && 'default' in subschema;
      if (hasOwnDefault) return subschema;
      const standardDefault = path__default["default"]([entName, propName], attributeDefaults);
      if (!standardDefault) return subschema;
      return { ...subschema, default: standardDefault };
    }),
  });
  const transformRelationshipFields = evolve__default["default"]({
    properties: map__default["default"](transformResourceSchema),
  });
  const transformAttributesSchema = compose__default["default"](
    transformAttributeFields,
    removeDrupalMetaSchemas(drupalMetaFields.attributes),
  );
  const transformRelationshipsSchema = compose__default["default"](
...
const transformLocalEntity = (entName, data) => compose__default["default"](
  dissoc__default["default"]('meta'),
  evolve__default["default"]({
    type: t => `${entName}--${t}`,
    attributes: { timestamp: dropMilliseconds },
    relationships: map__default["default"](r => ({ data: r })),
  }),
)(data);

const transformRemoteAttributes = compose__default["default"](
  evolve__default["default"]({ timestamp: safeIso }),
  omit__default["default"](drupalMetaFields.attributes),
);
const transformRemoteRelationships = compose__default["default"](
  map__default["default"](prop__default["default"]('data')),
  omit__default["default"](drupalMetaFields.attributes),
);
const makeFieldChanges = (attrs, rels) => ({
  ...map__default["default"](() => safeIso(attrs.changed), omit__default["default"](drupalMetaFields.attributes, attrs)),
  ...map__default["default"](() => safeIso(rels.changed), omit__default["default"](drupalMetaFields.relationships, rels)),
});
...
const transformRemoteEntity = (entName, setLastSync = false) => ({
  ...
  meta: {
    ...
    remote: {
      ...
      meta: {
        attributes: pick__default["default"](drupalMetaFields.attributes, attributes),
        relationships: pick__default["default"](drupalMetaFields.relationships, relationships),
      },
    },
  },
...
const transformFetchResponse = name => evolve__default["default"]({
  data: map__default["default"](transformRemoteEntity(name)),
});

const transformSendResponse = name => compose__default["default"](
...
const aggregateBundles = reduce__default["default"]((aggregate, result) => {
  const { reason, value, status } = result;
  if (status === 'fulfilled') {
    const nextData = chain__default["default"](path__default["default"](['data', 'data']), value);
    return evolve__default["default"]({
      data: concat__default["default"](nextData),
      fulfilled: concat__default["default"](value),
    }, aggregate);
  }
  return evolve__default["default"]({
...
function adapter(model, opts) {
  ...
  const chainRequests = (req, limit, prev = [], total = 0) => req.then((res) => {
    let next = path__default["default"](['data', 'links', 'next', 'href'], res);
    const resLength = path__default["default"](['data', 'data', 'length'], res);
  ...
  return {
    ...
    schema: {
      fetch(entName, type) {
        return connection.schema.fetch(entName, type)
          .then((schemata) => {
            if (!entName) {
              return mapObjIndexed__default["default"](
                (entitySchemata, entityName) => map__default["default"](
            ...
            if (!type) {
              return map__default["default"](transformD9Schema(entName), schemata);
            }
    ...
    ...entityMethods(({ nomenclature: { name, shortName } }) => ({
      ...
      fetch: ({ filter, limit }) => {
        ...
        const handleBundleResponse = compose__default["default"](
...
function model(options = {}) {
  ...
  function getSchemata(entity, type) {
    ...
    return clone__default["default"](schemata[entity][type]);
  }
...
function farmOS(farmConfig) {
  ...
  const addRemote = (remoteConfig = {}) => {
    ...
    farm.remote = { ...omit__default["default"](shortNames, connection), add: addRemote };
  };
